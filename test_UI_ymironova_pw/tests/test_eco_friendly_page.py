def test_add_to_cart(create_eco_friendly_page):
    ...
    create_eco_friendly_page.check_page_header_title_is(
        "Eco Friendly"
    )  # проверяем заголовок страницы
    item_name = (
        create_eco_friendly_page.choose_item()
    )  # выбираем размер, цвет товара и запоминаем его название
    create_eco_friendly_page.open_cart()  # переходим в корзину
    create_eco_friendly_page.check_page_header_title_is(
        "Shopping Cart"
    )  # проверяем заголвок страницы
    create_eco_friendly_page.check_item_in_cart(
        item_name
    )  # сравниваем имя товара в корзине и сохраненное
...
def test_remove_item_from_cart(create_eco_friendly_page):
    ...
    create_eco_friendly_page.check_item_in_cart_icon(
        item_name
    )  # сравниваем имя товара в корзине и сохраненное
